...
async def get_keys(stale = False):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.is_success:
            return response.json()
        raise OAuthKeysUnavailableException(f"Cannot get keyset from OAuth server {response}")
...
async def endpoint_validate_ouath_code(request: Request):
    ...
    response_body = await exchange_code(config.oauth_token_url, code, config.oauth_client_id, config.oauth_client_secret, config.oauth_redirect_uri)    
    print(response_body)
    ...
    response.set_cookie(key="als_token", value=encoded_jwt, httponly=True, max_age=86400)
    return response
